...
def percentage_change(crypto):
    ...
    # iterate through all the table rows and get the required data
    for table_row in table_rows:
        name_element = table_row.find(
            "p", class_="sc-71024e3e-0 ehyBa-d"
        )  # Replace with correct class if needed
        if name_element and name_element.text.strip().lower() == crypto.lower():
            percentage_changes = table_row.find_all(
                "span", class_=["sc-a59753b0-0 cmnujh", "sc-a59753b0-0 ivvJzO"]
            )
            ...
            if len(percentage_changes) >= 3:  # Ensure there are at least 3 changes
                ...
                twenty_four_hour_change = extract_change_with_symbol(
...
for crypto in cryptos:
    ...
    one_hour_change, twenty_four_hour_change, seven_day_change = percentage_change(
        crypto
    )
    data = {
        "Name": name,

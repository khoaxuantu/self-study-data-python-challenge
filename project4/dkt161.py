# NOTE: Using argparse to get the file path from the user instead of hardcoding it, because not everyone has the same file path.
...
def automated_crypto_scraping(file_path=""):
    ...
    try:
        ...
        table = BeautifulSoup(page.text, "html").find(
            "table", class_="sc-7b3ac367-3 etbcea cmc-table"
        )
        ...
        def data_cleaner(df):
            ...
            df[["Volume(24h)_USD", "Volume(24h)_Currency"]] = df["Volume(24h)"].apply(
                lambda x: pd.Series(split_volume(x))
            )
            df.drop(
                columns=["Volume(24h)"], inplace=True
            )  # Remove the original column after use

            # Remove $ and , from the 'Price' column
            df["Price"] = (
                df["Price"].str[1:].str.replace(",", "", regex=False).astype("float")
            )
            # Remove %
        ...
        if not os.path.exists(file_path + "crypto_scraped_data.csv"):
            ...
            df.to_csv(
                file_path + "crypto_scraped_data.csv",
                mode="a",
                header=False,
                index=False,
            )

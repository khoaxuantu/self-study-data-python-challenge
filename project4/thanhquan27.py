...
table = soup.find(
    "table"
)  # Get tag <table> trong HTML -> từ đây có thể lấy data -> đưa vào dataframe


# In[112]:


table_headers = table.find_all(
    "th"
)  # Get <th> trong table để làm headers trong dataFrame


# In[113]:


column_names = [columns.text for columns in table_headers][
    1:-1
]  # Không lấy cột đầu và cột cuối trong table của Website
...
for row in table_rows[
    1:
]:  # Bỏ qua tiêu đề bảng, lấy từ dòng thứ 2 trở đi trong table trên website
    ...
    if len(row_data) >= 9:  # Nếu số lượng cột >= 9, thực hiện xử lý
        ...
        for i in range(
            1, 10
        ):  # Duyệt qua các cột từ 1 đến 9 (đã loại bỏ cột số thứ tự)
            ...
            if i in [3, 4, 5]:  # Các cột 1h%, 24h%, 7d%
                ...
                if icon:
                    cell_data = (
                        "-" + cell_data
                    )  # Thêm dấu trừ (negative numbers) nếu có icon-Caret-down
        ...
        values_in_each_cell = [
            row_data[i].text.strip() if i < len(row_data) else "N/A"
            for i in range(1, 10)
        ]

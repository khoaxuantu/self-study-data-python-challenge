# Unrunnable code due to the class name has been changed.
...
def scrape_data(name):
    ...
    pct_1d = soup.find("p", class_="sc-71024e3e-0 sc-58c82cf9-1 bgxfSG iPawMI").text
    pct_1d = pct_clean_and_convert(
        pct_1d, soup.find("p", class_="sc-71024e3e-0 sc-58c82cf9-1 bgxfSG iPawMI")
    )
    pct_7d = soup.find("p", class_="sc-71024e3e-0 sc-58c82cf9-1 ihXFUo iPawMI").text
    pct_7d = pct_clean_and_convert(
        pct_7d, soup.find("p", class_="sc-71024e3e-0 sc-58c82cf9-1 ihXFUo iPawMI")
    )
    metrics = soup.find_all("dd", class_="sc-65e7f566-0 dzgtSD base-text")
    market_cap = metrics[0].text.split("%")[1]
    market_cap = price_clean_and_convert(market_cap, currency_str="$", split_str=",")

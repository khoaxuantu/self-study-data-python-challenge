# Yoor code is not runnable due to missing pandas library. Please make sure to include all necessary libraries in your code. And there is a big error in your code, which makes you do not have full marks.
...
def parse_crypto_data(soup):
    ...
    # Find all cryptocurrency rows -> This is where your code gets wrong, because there is only one element has this class. Thus your code just have one row Bitcoin.
    crypto_rows = soup.find_all(
        "div", class_="sc-7b3ac367-2 cFnHu"
    ...
    for row in crypto_rows:
        try:
            ...
            crypto_name = (
                crypto_name_element.text.strip() if crypto_name_element else "N/A"
            )
            ...
            market_cap = (
                market_cap_element.text.strip() if market_cap_element else "N/A"
            )

            # Extract the 24h volume
            volume_24h_element = row.find(
                "p", class_="sc-71024e3e-0 bbHOdE font_weight_500"
            )
            volume_24h = (
                volume_24h_element.text.strip() if volume_24h_element else "N/A"
            )

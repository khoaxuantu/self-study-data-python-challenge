...
def fetch_crypto_data(crypto_name, crypto_abbr):
    ...
    for row in coin_row:
        ...
        if crypto_name_data and input.lower() in crypto_name_data[0].lower():
            ...
            market_cap = (
                f"${market_cap_volume[1]}" if len(market_cap_volume) > 1 else None
            )
            volume_24h_elements = row.find_all(
                "p", {"class": "sc-71024e3e-0 bbHOdE font_weight_500"}
            )
            volume_24h = volume_24h_elements[0].text

            # Prepare the data to be added to DataFrame
            current_price = individual_row_data[2:7] + [
                market_cap,
                volume_24h,
                timestamp,
            ]

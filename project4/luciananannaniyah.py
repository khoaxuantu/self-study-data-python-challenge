...
def fetch_data(cryptocurrencies):
    ...
    header_filtered = [value for i, value in enumerate(header) if i not in exclude_indices]
...
while True:
    ...
    table_data_with_timestamp = [table_data[0]] + [row + [timestamp] for row in table_data[1:]]
    df = pd.DataFrame(table_data_with_timestamp[1:], columns=table_data_with_timestamp[0])

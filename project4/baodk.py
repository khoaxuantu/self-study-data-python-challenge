        ...
        while not stopEvent.is_set():
            try:
                ...
                if response and response.status_code == 200:
                    headers, rows = parse_table_html(
                        response.text, tsNow, allowedCoinNames
                    )
                    writer, file = create_csv_writer(fileName, headers)
                    columnWidths = [len(h) for h in headers]
                    for row in rows:
                        columnWidths = [
                            max(o, len(c)) for o, c in zip(columnWidths, row)
                        ]
                        writer.writerow(new_dict(headers, row))
                    file.close()
                    formattedHeaders = "|".join(
                        [
                            " {:^{width}} ".format(h, width=colWidth)
                            for h, colWidth in zip(headers, columnWidths)
                        ]
                    )
                    ...
                    formattedRows = "\r\n".join(
                        [
                            "|".join(
                                [
                                    " {:<{width}} ".format(cell, width=colWidth)
                                    for cell, colWidth in zip(row, columnWidths)
                                ]
                            )
                            for row in rows
                        ]

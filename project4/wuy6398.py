...
def data_append(row):
    ...
    """
    datas = [data for data in row.stripped_strings] + [datetime.now().strftime("%Y-%m-%d %H:%M:%S.%f")]
    append_data = [datas[i] for i in data_index.values()]
    return append_data

print('Collect data on Bitcoin, Ethereum, and BNB from CoinMarketCap in a desired time length with desired intervals.')

#Nhập đầu vào tổng khoảng thời gian và khoảng cách giữa các đợt lấy data theo mong muốn 
while True:
    try:
        length = float(input('Input the time length you want to scrape the data (in minutes):'))
        interval = float(input('Input the interval between the data collected (in seconds)(>= 1):'))
        if length*60 <= interval or interval < 1:
            raise InputError('Invalid input! Try again.\nMake sure your time length in seconds is longer than your interval and your interval is larger than 1 second!\n')
        break

...
while True:
    try:
        ...
        more_button = driver.find_element(
            By.XPATH, '//*[@id="tab-job"]/div/div/div/button'
        )
        more_button.click()
        time.sleep(2)
    except:
...
for row in job_rows:
    ...
    if job_link_true:
        ...
        job_page_url = requests.compat.urljoin(
            url, job_link
        ...
        if (
            "junior" not in job_general_info_lower
            and "fresher" not in job_general_info_lower
        ):
            continue
        ...
        if detail_job_header:
            ...
            time_posted_tag = detail_job_header.find(
                class_="flex w-11/12 items-center text-base text-gray-500"
            )
            time_posted = time_posted_tag.get_text().strip()
            time_posted_datetime = parse_relative_time(time_posted)
            time_posted = time_posted_datetime.strftime(
                "%d/%m/%Y"

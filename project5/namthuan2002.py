...
def get_current_time(string):
    ...
    if re.search(r"hour", string, re.I):
        result = datetime.datetime.now() - datetime.timedelta(
            hours=int(re.findall(r"\d+", string)[0])
        )
        ...
        result = datetime.datetime.now() - datetime.timedelta(
            weeks=int(re.findall(r"\d+", string)[0])
        )
    elif re.search(r"min..", string, re.I):
        result = datetime.datetime.now() - datetime.timedelta(
            minutes=int(re.findall(r"\d+", string)[0])
        )
...
def crawlData_from_ITViec(page_num=1):
    driver = uc.Chrome()
    url = "https://itviec.com/it-jobs?page=" + str(
        page_num
    )  # ?job_selected=software-engineer-java-english-fortna-5335"
    ...
    for j in jobs_cre:
        ...
        job_link = (
            "https://itviec.com"
            + j.find("a", {"data-search--job-selection-target": "jobTitle"})["href"]
        )
        label_link = "Job link"
        ...
        try:
            job_salary = (
                sub_page.find("div", class_="job-header-info").find("span").get_text()
            )
        except:
        ...
        try:
            job_skill = job_preview.find(
                "div", class_="d-flex align-items-center gap-1"
            ).get_text(separator=",")
        except:
        ...
        try:
            job_reason = job_preview.find("section", class_="reasons-join-us").get_text(
                separator="\n"
            )
        except:
        ...
        try:
            job_desc = job_preview.find("section", class_="job-description").get_text(
                separator="\n"
            )
        except:
        ...
        try:
            job_exper = job_preview.find("section", class_="job-experiences").get_text(
                separator="\n"
            )
        except:
        ...
        try:
            job_benefit = job_preview.find(
                "section", class_="job-why-love-working"
            ).get_text(separator="\n")
        except:
...
def job():
    try:
        df = pd.DataFrame(
            crawlData_from_ITViec(page_num=1) + crawlData_from_ITViec(page_num=2)
        )
        df.to_excel(r"F:\My_Learning\Python\Challenges\List Jobs.xlsx")

        # Filter Python or SQL or C++ skill job
        df = df[
            df["Required skill"].apply(
                lambda x: True if re.search(r"python|SQL|C\+\+", x, re.I) else False
            )
        ]
        # Filer on-site job
        df = df[
            df["Type"].apply(
                lambda x: True if re.search(r"(office)|(on.*site)", x, re.I) else False
            )
        ]
        # Filter jobs in HCM city
        df = df[
            df["Location"].apply(
                lambda x: True if re.search(r"h.*c.*m.*", x, re.I) else False
            )
        ]
        # # Filter out intern job
        df = df[
            df["Job name"].apply(
                lambda x: False if re.search(r"intern|t.*t.*s.*", x, re.I) else True
            )
        ]

...
def filter_jobs(jobs, include_keywords=[], exclude_keywords=[]):
    filtered_jobs = []
    for job in jobs:
        include = any(re.search(keyword, job['Description'], re.IGNORECASE) for keyword in include_keywords)
        exclude = any(re.search(keyword, job['Description'], re.IGNORECASE) for keyword in exclude_keywords)
...
def main():
    ...
    if html_content:
        ...
        filtered_jobs = filter_jobs(jobs, include_keywords=['Python', 'JavaScript'], exclude_keywords=['Internship', 'Part-time'])
...
def main():
    ...
    filtered_jobs = filter_jobs(all_jobs, include_keywords=['Python', 'JavaScript'], exclude_keywords=['Internship', 'Part-time'])
    save_to_csv(filtered_jobs)
...
async def job_scrape():
    ...
    for _, job in jobs_df.iterrows():
        await channel.send(f"**{job['Job Title']}** at {job['Company']} in {job['Location']}\n{job['Link']}")

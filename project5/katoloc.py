# Great work! I did comment on the code. Please check the comments (find NOTE:) and let me know if you have any questions. And seems like you did not implement the pagination feature.
...
# NOTE: Please read https://stackoverflow.com/questions/28377421/why-do-i-receive-a-timeout-error-from-pythons-requests-module
...
def send_mail():
    ...
    list_path = [
        path + "Web My Job Mag" + "/MyJobMag.csv",
        path + "Web It Job Pro" + "/Itjobpro.csv",
    ]
    flag = 0
    for i in list_path:
...
list_keywords = []
tmp = input(
    'Enter certain keywords that Jobs include (e.g., "remote", "full time")(#:end): '
)
while tmp != "#":
...
def my_job():
    ...
    for url in list_url:
        ...
        tag = re.findall(
            r"<ul class=\"(?:job_listings|job-list)(?:\s[^\"]*)?\">(.*?)</ul>\n(\n|<div class)",
            page.text,
            re.DOTALL,
        ...
        ]  # list tat ca folder tai noi file py dang o
        # cu moi <li> lay link cua job do
        for i in re.findall(r"<a href=\"(.*?)\">[a-zA-Z]", tag[0][0], re.DOTALL):
            tmp = re.search(
                r"<a href=\"(.*?)$", i, re.DOTALL
            )  # regex k tot nen loc 1 lan nx de lay dung link job
            if tmp:
        ...
        if flag == 1:
            ...
            for link in list_link:
                ...
                with open(
                    path + "Web It Job Pro" + "/" + job_title + ".txt",
                    "w",
                    newline="",
                    encoding="UTF8",
                ) as f:
                ...
                for language in list_languages:
                    ...
                    if re.search(
                        language,
                        taglink.text + souplink.find("div", "job_description").text,
                        re.IGNORECASE,
                    ):
                        note = "#"
            ...
            for link in list_link:
                ...
                company_name = re.search(r"\bat.*$", taglink.find("h1").text).group()[
                    2:
                ]
                ...
                with open(
                    path + "Web My Job Mag" + "/" + job_title + ".txt",
                    "w",
                    newline="",
                    encoding="UTF8",
                ) as f:
...
while True:
    my_job()
    # NOTE: Consider using another way to schedule the task, such as schedule library.
    time.sleep(
        90000
    )  # 1 ngay = 86400s ma time.sleep() thuong co thoi gian thuc te it hon thoi gian minh yeu cau(su lap lich cua moi he thong khac nhau)-> lay 90000

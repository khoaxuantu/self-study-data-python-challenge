...
def crawl_page (page_url, page_number, retries=3, backoff_factor=2):
    ...
    for block in title_blocks:
        ...
        job_detail = {'Job description': 'Mô tả công việc'
            ...
            try:
                ...
                for key, value in job_detail.items():
                    ...
                    if (section == None):
                        ...
                        try:
                            section_h2 = detail_job_content.find('h2', class_='title', text = value)
                            content = section_h2.find_next ('div') #, class_='content-tab'
                            job_detail [key] = content.get_text(separator="\n", strip=True)
                        except:
                            job_detail [key] = 'None'
                    else:
                        content = section.find_next('div', class_='job-description__item--content')
                        job_detail [key] = content.get_text(separator="\n", strip=True)
                try:
                    employment_type_tmp = detail_job_content.find('strong', text='Hình thức làm việc ')
                    employment_type = employment_type_tmp.find_next('span').get_text(strip=True)
                except:
                    employment_type_tmp = detail_job_content.find('div', class_='box-general-group-info-title', text='Hình thức làm việc')
                    employment_type = employment_type_tmp.find_next_sibling('div', class_='box-general-group-info-value').get_text(strip=True)
                ...
                if attempt < retries - 1:
                    ...
                    print(f"Failed to fetch job details for {job_link} after {retries} attempts: {e}")
                    continue
        # 1.4. Add job items to job list
        try:
            job_item = { 'Job title': job_title
    ...
    df = pd.DataFrame (job)
    df['Job id'] = df['Job link'].apply(lambda x: re.search(r'(\d+)\.html?', x).group(1))
    if page_number == 1:

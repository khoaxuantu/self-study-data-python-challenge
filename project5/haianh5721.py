...
def filter_jobs(job_listings, include_keywords, exclude_keywords):
    ...
    for job in job_listings:
        include_match = any(re.search(keyword, job['description'], re.IGNORECASE) for keyword in include_keywords)
        exclude_match = any(re.search(keyword, job['description'], re.IGNORECASE) for keyword in exclude_keywords)
...
def save_to_csv(job_listings, filename):
    with open(filename, 'w', newline='', encoding='utf-8') as csvfile:
        fieldnames = ['title', 'company', 'location', 'description', 'date_posted', 'job_link']

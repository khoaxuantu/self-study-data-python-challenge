...
def fetch_job_details(link):
    try:
        ...
        job_type_element = soup.find('div', class_='item-card-info mb-2 w-1/2 md:mb-4 md:w-full')
        job_type = job_type_element.get_text(strip=True) if job_type_element else 'N/A'
        
        contract_type_element = soup.find('div', class_='item-card-info mb-2 w-1/2 pl-3 md:mb-4 md:w-full md:pl-0')
        contract_type = contract_type_element.get_text(strip=True) if contract_type_element else 'N/A'
        ...
        tech_stack = ', '.join([tech.get_text(strip=True) for tech in tech_stack_element.find_all('a')]) if tech_stack_element else 'N/A'
...
def parse_job_element(job):
    ...
    company_element = job.find('div', class_='mt-1').find('a') if job.find('div', 'mt-1') else None
    company = company_element.get_text(strip=True) if company_element else 'N/A'
    
    location_div = job.find('div', class_='flex flex-wrap items-end gap-2 text-gray-500')
    location = ', '.join([loc.get_text(strip=True) for loc in location_div.find_all('p')]) if location_div else 'N/A'
...
def main():
    ...
    if file_exists:
        ...
        combined_df.drop_duplicates(subset=['Title', 'Company'], keep='last', inplace=True)
        

        combined_df.to_csv('filtered_job_listings.csv', index=False, encoding='utf-8-sig')
    else:

        new_df.to_csv('filtered_job_listings.csv', mode='w', header=True, index=False, encoding='utf-8-sig')

...
for job in soup.find_all('li','mb-4 last:mb-0'):
    try:
        job_title_tags = job.find_all('a', class_=['text-lg font-bold transition-all text-primary', 'text-lg font-bold transition-all hover:text-primary'])

        # Lấy nội dung văn bản từ các thẻ <a>
        job_title = ' '.join([tag.text for tag in job_title_tags])

        company_name = job.find('a', 'text-gray-600 transition-all hover:text-primary').text
        ...
        links = soup1.find_all('a', class_='text-sm hover:text-primary-300 hover:underline md:text-base')[1:]
...
def send_email(to_email, subject, body):
    ...
    server.sendmail('<<EMAIL_MASK>>', to_email, msg.as_string())#(sender_email, receiver_email, message.as_string())
    server.quit()

to_email = input("Enter your email address to receive notifications: ")

keyword = input("Enter the one or more keyword to filter jobs (e.g.fulltime tester or junior html): ")

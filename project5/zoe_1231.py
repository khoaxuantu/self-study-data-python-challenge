...
def automated_job_listing():
    ...
    # Lấy Job title
    job_title_columns = soup.find_all('a', class_ = ['text-lg font-bold transition-all hover:text-primary', 'text-lg font-bold transition-all text-primary'])
    job_title = [column.text for column in job_title_columns]
    
    # Lấy Company name
    company_name_columns = soup.find_all('a', class_ = 'text-gray-600 transition-all hover:text-primary')
    company_name = [column.text for column in company_name_columns]
    
    # Lấy Job location
    job_location_columns = soup.find_all('div', class_ = 'flex flex-wrap items-end gap-2 text-gray-500')
    ...
    skill_groups = [ [a.text for a in div.find_all('a', class_='mr-2 inline-block')] for div in divs if div.find_all('a', class_='mr-2 inline-block') ]
    
    # Lấy Date posted
    date_posted_columns = soup.find_all('p', class_ = 'whitespace-nowrap text-sm text-gray-400')
    date_posted = [column.text for column in date_posted_columns]
    
    # Lấy Job link
    lis = soup.find_all('li', class_='mb-4 last:mb-0')
    job_links = [url_topdev + li.find('a', href=True)['href'] for li in lis if li.find('a', href=True)]
    ...
    filter_choice = input("Do you want to filter the result? (yes/no): ").strip().lower()
    ...
    if filter_choice == 'yes':
        ...
        df = df[df['Skills'].apply(lambda x: bool(re.findall(filter_pattern, x, flags=re.IGNORECASE)))]
    ...
    csv_file_path = r'C:\Users\Admin\Python 35-days Challenge\Project 5\job_listings.csv'
    
    if os.path.exists(csv_file_path):
        df.to_csv(csv_file_path, encoding='utf-8-sig', mode='a', header=False, index=False)
    else: 

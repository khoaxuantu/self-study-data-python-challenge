# Great work! I did comment on the code. Please check the comments (find NOTE:) and let me know if you have any questions.
...
def send_email_with_mailgun(subject, body, to_email, file_path):
    ...
    with open(file_path, "rb") as attachment:
        files = {"attachment": (file_path, attachment.read())}
        ...
        response = requests.post(url, auth=("api", api_key), files=files, data=data)

    if response.status_code == 200:
        print("Email đã được gửi thành công!")
    else:
        print(
            f"Gửi email không thành công. Mã lỗi: {response.status_code}, Nội dung: {response.text}"
        )


def send_discord_message(message, file_path):
    webhook_url = (
        "<<DISCORD_WEBHOOK_URL_MASK>>"  # Don't public your discord app webhook
    )
    with open(file_path, "rb") as file:
        data = {"content": message, "username": "Your Bot Name"}
...
def fetch_jobs(num_pages, languages, required_keywords, excluded_keywords, to_email):
    ...
    # NOTE: Where do you assign the page variable into the url? It seems like the page is always 2
    # So you dont have any point for the pagination
        ...
        if response.status_code == 200:
            ...
            if data["data"]:
                ...
                for job in jobs:
                    ...
                    description = fetch_job(
                        "https://topdev.vn/detail-jobs/-" + str(job["id"])
                    )
    ...
    bodyMail = (
        ...
        "Có chứa các Ngôn ngữ lập trình: "
        + ", ".join(language_keywords)
        + " | Chứa các từ khoá: "
    ...
    send_email_with_mailgun(
        "Thu thập dữ liệu từ website Topcv.vn " + current_time,
        bodyMail,
        to_email,
        file_path,
    )

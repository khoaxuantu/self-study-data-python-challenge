...
def job_scraper():
    ...
    for row in column1:
        ...
        date_container = row.find(
            "div", class_="mt-4 flex items-center justify-between"
        )
        date_posted = (
            date_container.find("p").text.strip()
            if date_container and date_container.find("p")
            else None
        )
...
criteria = {"Level": "Fresher|Intern", "Location": "^[Hồ Chí Minh]%"}

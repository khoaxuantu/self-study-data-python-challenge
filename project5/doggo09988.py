...
def scrawl_data(jds):
    for jd in jds:
        if(jd.find('a')):
            if 'hours' in jd.find('p', class_="whitespace-nowrap text-sm text-gray-400").text:
                ...
                link = new_url
                company = soup.p.text
                location = soup.find('div', class_='w-3/4 flex flex-initial flex-col').div.div.div.text.split(", ")[-1]
...
jds=soup.main.find('ul', class_='mt-4').find_all('div', recursive=False)[-1].ul.find_all('li',  recursive=False)

        ...
        if job_data:
            ...
            for job in job_items:
                try:
                    ...
                    company_name = (
                        job.find("div", class_="sc-hybRYi iVjhps").text.strip()
                        if job.find("div", class_="sc-hybRYi iVjhps")
                        else None
                    )
                    location = (
                        job.find("span", class_="sc-evdWiO hRDCRg").text.strip()
                        if job.find("span", class_="sc-evdWiO hRDCRg")
                        else None
                    )
                    date_posted = (
                        job.find("div", class_="sc-lmJFLr eJkRTg").text.strip()
                        if job.find("div", class_="sc-lmJFLr eJkRTg")
                        else None
                    )
                    salary = (
                        job.find("span", class_="sc-gQSkpc cXNeHq").text.strip()
                        if job.find("span", class_="sc-gQSkpc cXNeHq")
                        else "Thương lượng"
                    )
                    skills = job.find_all(
                        "label", class_="sc-dxcDKg RKGYC sc-ERObt jKzuwy"
                    )
                    list_skills = [skill.text for skill in skills]
                    job_link = (
                        "https://www.vietnamworks.com"
                        + job.find("a", href=True)["href"]
                        if job.find("a", href=True)
                        else None
                    )

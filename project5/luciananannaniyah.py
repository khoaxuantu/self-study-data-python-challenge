...
CRITERIA = {"include": [r"\bPython\b"], "exclude": [r"\bHybrid\b"]}
...
def filter_job(text):
    include_patterns = [
        re.compile(pattern, re.IGNORECASE) for pattern in CRITERIA["include"]
    ]
    exclude_patterns = [
        re.compile(pattern, re.IGNORECASE) for pattern in CRITERIA["exclude"]
    ]

    if any(pattern.search(text) for pattern in include_patterns) and not any(
        pattern.search(text) for pattern in exclude_patterns
    ):
        return True
    else:
        return False
    ...
    while True:
        ...
        for item in job_cards:
            try:
                ...
                job_company = (
                    item.find("div", class_="imy-3").find("span").find("a").text.strip()
                )
                job_location = (
                    item.find("svg", class_="feather-icon")
                    .find_parent("div")
                    .find("span")
                    .text.strip()
                )
                job_date = (
                    item.find("div", class_="d-flex").find("span").text.strip()[7:]
                )
                job_type = item.find("g").find_parent("div").find("span").text.strip()
                job_tags = ", ".join(
                    tag.text.strip() for tag in item.find_all("div", class_="itag")
                )
                ...
                if description_tag:
                    job_description += ". ".join(
                        li_tag.text.strip() for li_tag in description_tag.find_all("li")
                    )

...
def fetch_job_details(job_id):
    ...
    job_description = soup.select_one('#JobDescription .prose').get_text(separator=" ").replace("  ", " ")
    ...
    for page in range(1, num_pages + 1):
        ...
        if response.status_code == 200:
            ...
            if 'data' in data and data['data']:
                jobs = data['data']
                for job in jobs:
                    job_id = job.get('id')  # Sử dụng get() để lấy giá trị id an toàn hơn
                    if job_id:
                        description = fetch_job_details(job_id)
                        description += job.get('job_levels_str', '') + job.get('skills_str', '')
                        ...
                        if (language_regex.search(description) and
                            ...
                            job_listings.append({
                                ...
                                'Location': job.get('addresses', {}).get('full_addresses', ['Unknown'])[0],
                                'Description': description,
                                'Date Posted': job.get('published', {}).get('date', 'Unknown'),
...
def run_job_search(num_pages, language_keywords, include_keywords, exclude_keywords, to_email):
    print('Start running')
    job_listings = fetch_jobs(num_pages, language_keywords, include_keywords, exclude_keywords)
    if job_listings:
        ...
        send_email_with_smtp(f"Thu thập dữ liệu từ website Topcv.vn {current_time}", bodyMail, to_email, file_path)
...
schedule.every().day.at(run_time).do(lambda: run_job_search(num_pages, language_keywords, include_keywords, exclude_keywords, to_email))
while True:
    current_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")

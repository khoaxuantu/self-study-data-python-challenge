# Great work! But seems like u did not implement pagination and schedule features.
...
class TopDevScraper:
    ...
    def scrape_job_details(self, job_links):
        ...
        for job_link in job_links:
            ...
            try:
                job_title = soup_url.find(
                    "h1", class_="text-2xl font-bold text-black"
                ).get_text()
                company_name = soup_url.find(
                    "p", class_="my-1 line-clamp-1 text-base font-bold text-gray-500"
                ).get_text()
                job_location = soup_url.find("div", class_="w-11/12").get_text()
                post_time_ago = soup_url.find(
                    "div", class_="flex w-11/12 items-center text-base text-gray-500"
                ).get_text()[7:]
                post_time = self.parse_time(post_time_ago)
                experience = soup_url.find_all(
                    "a",
                    class_="text-sm hover:text-primary-300 hover:underline md:text-base",
                )[0].get_text()
                job_description_text = soup_url.find(
                    "div", class_="rounded bg-white p-4 md:px-6 md:py-4"
                ).get_text()
                job_description = (
                ...
                level_information = soup_url.find(
                    "div",
                    class_="item-card-info mb-2 w-1/2 pl-3 md:mb-4 md:w-full md:pl-0",
                )
                level = (
                    [
                        a_tag.get_text()
                        for a_tag in level_information.find_all(
                            "a",
                            class_="text-sm hover:text-primary-300 hover:underline md:text-base",
                        )
                    ]
                ...
                contract_type = None
                contract_type_information = soup_url.find_all(
                    "div",
                    class_="item-card-info mb-2 w-1/2 pl-3 md:mb-4 md:w-full md:pl-0",
                )
                for info in contract_type_information:
                    contract_type_tag = info.find(
                        "a",
                        class_="text-sm hover:text-primary-300 hover:underline md:text-base",
                    )
                ...
                tech_stack = [
                    ...
                    for info in soup_url.find_all(
                        "span",
                        class_="whitespace-nowrap rounded border border-solid font-normal transition-all inline-flex items-center justify-center border-blue-light text-blue-dark bg-blue-light hover:border-blue-dark h-[1.625rem] px-2 text-xs md:h-7 md:px-2 md:text-sm",
                    )
                ]
                job_content = (
                    job_title
    ...
    def filter_jobs(self, job_list):
        ...
        for job in job_list:
            include_match = any(
                re.search(pattern, job, re.IGNORECASE) for pattern in include_patterns
            )
            keyword_match = any(
                re.search(pattern, job, re.IGNORECASE) for pattern in keyword_patterns
            )
            exclude_match = any(
                re.search(pattern, job, re.IGNORECASE) for pattern in exclude_patterns
            )
...
if __name__ == "__main__":
    ...
    scraper = TopDevScraper(
        driver_path="D:/Drivers/chromedriver-win64/chromedriver.exe",

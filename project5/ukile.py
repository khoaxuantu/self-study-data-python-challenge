# # Great work! But seems like u did not implement schedule and mail. I commented on the code, please find NOTE: to see my comments.
...
# NOTE: You should also handle the case the page does not exist.
for page in range(1, 6):  # Modify the range as needed
    ...
    job_listings = soup.find_all(
        "div", class_="job__body"
    )  # Adjust the class name as needed

    for job in job_listings:
        title = job.find("h3", class_="title").get_text(
            strip=True
        )  # Adjust the tag and class name as needed
        company = job.find("div", class_="company").get_text(
            strip=True
        )  # Adjust the class name as needed
        location = job.find("div", class_="location").get_text(
            strip=True
        )  # Adjust the class name as needed
        description = job.find("div", class_="description").get_text(
            strip=True
        )  # Adjust the class name as needed
        date_posted = job.find("div", class_="date").get_text(
            strip=True
        )  # Adjust the class name as needed

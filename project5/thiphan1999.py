# Great job! But seems like u did not implement the mail feature. I commented on the code, please find NOTE: to see the comments.
...
def region_params(region=None):
    if region == "Hồ Chí Minh" or region == "Hà Nội" or region == "Đà Nẵng":
        choose = region.title()
        regions = {"Hồ Chí Minh": "79", "Hà Nội": "01", "Đà Nẵng": "48"}
        params = {
            "region_ids": None,
            "ordering": "newest_job",
            "page": 1,
...
regions = input(
    "Chọn tên khu vực (Hồ Chí Minh | Đà Nẵng | Hà Nội |All Location): "
).title()
start_page = int(input("Trang bắt đầu: "))
end_page = int(input("Trang kết thúc: "))
...
def get_json_data():
    ...
    all_job_lists = []
    # NOTE: You should also handle when the page does not exist.
...
filter_jobs = input(
    f"Nhập từ khóa công việc muốn tiềm kiếm ở {regions} hoặc để trống: "
)
...
def job_information():
    ...
    for detail_url in get_detail_job_links():
        ...
        job_description = (
            soup.find("div", class_="prose max-w-full text-sm text-black lg:text-base")
            .find("span")
            .text
        )

        year_of_experience = (
            soup.find("div", class_="flex flex-wrap")
            .find("div", class_="item-card-info mb-2 w-1/2 md:mb-4 md:w-full")
            .find("a")["title"]
        )
        lists_level = (
            soup.find("div", class_="flex flex-wrap")
...
def create_csv():
    if get_job_date() is None or get_detail_job_links() is None:
        ...
        df["job_time_posted"] = pd.to_datetime(
            df["web_scraping_date"]

...
table_column_titles = ['Job Title','Company Name', 'Job Location', 'Job Description', 'Date Posted', 'Job Link','Experience','Level','Job Type','Contract Type','Tech Stack']
...
def automated_job_data_pull(x):
    ...
    #Job filter:
    filter_general=soup2.find('div',class_="container flex flex-wrap items-start gap-6 px-0 md:flex-nowrap md:px-4")
    # Tech Stack Keyword
    ...
    job_name=soup2.find('p', class_='my-1 line-clamp-1 text-base font-bold text-gray-500').text.strip()
    job_location=soup2.find('div', class_='my-2 max-w-[540px] text-base text-gray-500').text.strip()
    job_des=soup2.find('div', id='JobDescription').text.strip()
    job_posted=soup2.find('div', class_='mb-2 max-w-[540px] text-base text-gray-500').text.strip()
...
for i in range(num_criteria):
    ...
    type_of_filter=input("Type of filter ? include keyword (I) or exclude keywords (E)")
...
for column, (key, type_of_filter) in filter.items():
    if type_of_filter == 'I':
        filter_dm = filter_dm[filter_dm[column].str.contains(key, case=False, na=False, regex=True)]
    elif type_of_filter == 'E':
        filter_dm = filter_dm[~filter_dm[column].str.contains(key, case=False, na=False, regex=True)]

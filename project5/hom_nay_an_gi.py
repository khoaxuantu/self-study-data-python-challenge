...
for job in job_listings:
    ...
    # Apply regex filters
    if (programming_languages.search(job_description) and
        keywords.search(job_description) and
        not exclude_terms.search(job_description)):
...
with open('filtered_jobs.csv', 'w', newline='', encoding='utf-8') as csvfile:
    fieldnames = ['Job Title', 'Company Name', 'Job Location', 'Job Description', 'Date Posted', 'Job Link']
